    ...
    class Sprockets
      ...
      def self.install(env)
        if ::Sprockets::VERSION.to_f < 4
          env.register_preprocessor('text/css', ::CSSM::Rails::Sprockets)
        else
          env.register_bundle_processor('text/css', ::CSSM::Rails::Sprockets)
        end
      end
      ...
      def self.uninstall(env)
        if ::Sprockets::VERSION.to_f < 4
          env.unregister_preprocessor('text/css', ::CSSM::Rails::Sprockets)
        else
          env.unregister_bundle_processor('text/css', ::CSSM::Rails::Sprockets)
        end
      end
